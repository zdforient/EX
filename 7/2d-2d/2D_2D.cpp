...
int main(int argc, char** argv){
    ...
    if(argc != 3){
        ...
        img_1 = imread(argv[1], CV_LOAD_IMAGE_COLOR);
        img_2 = imread(argv[2], CV_LOAD_IMAGE_COLOR);
    }
    
    if(img_1.empty() || img_2.empty()){
        cerr << "img file not loaded."<<endl;
        return 1;
    }
    ...
    cout<<"found features "<<matches.size()<<endl;
...
void find_feature_matches(const Mat &img_1, const Mat &img_2,
    ...
    //1. compute Oriented FAST conner points
    detector->detect(img_1, keypoints_1);
    detector->detect(img_2, keypoints_2);
    
    //2. corner BRIEF decriptor computation
    ...
    //set 30 or 2 * min_dist as boundary
    for(int i = 0; i < descriptors_1.rows; i++){
...
void pose_estimation_2d2d(std::vector<KeyPoint> keypoints_1, std::vector<KeyPoint> keypoints_2, std::vector<DMatch>& matches, Mat &R, Mat &t){
    ...
    // fundamental_matrix: point of object in camera coordinates
    Mat fundamental_matrix;
    fundamental_matrix = findFundamentalMat(points1, points2, CV_FM_8POINT);
    cout << "fundamental_matrix is \n"<<fundamental_matrix<<endl;
    
    //essential matrix: point of object in image
